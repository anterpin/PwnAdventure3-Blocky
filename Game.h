...
class GameAPI {
 ...
 public:
  ...
  class std::vector<IAchievement*, std::allocator<IAchievement*> >
  GetAchievementList();
  class std::vector<ItemPickup*, std::allocator<ItemPickup*> >
  GetGoldenEggList();
  size_t GetGoldenEggCount();
  virtual bool SpawnActor(class IActor*,
			  const struct Vector3&,
			  const struct Rotation&,
			  bool);
  virtual bool SpawnActorAtNamedLocation(class IActor*, const char*);
  virtual bool SpawnRemotePlayer(class IPlayer*,
				 const struct Vector3&,
				 const struct Rotation&);
  virtual void DamageInRadius(class IActor*,
			      class IItem*,
			      const struct Vector3&,
			      float,
			      int32_t,
			      enum DamageType);
  virtual size_t GetNamedLocationPoints(const char*,
					struct LocationAndRotation*&);
  virtual void FreeNamedLocationPoints(struct LocationAndRotation*);
  class std::vector<LocationAndRotation, std::allocator<LocationAndRotation> >
  GetNamedLocationPointList(const char*);
  bool GetNamedLocationPoint(const std::string&, struct LocationAndRotation&);
  class std::vector<LocationAndRotation, std::allocator<LocationAndRotation> >
  GetSpawnPoints(const char*);
  ...
  virtual void OnWeaponFired(class IItem*,
			     const struct Vector3&,
			     const struct Vector3&);
  virtual void OnBulletHitActor(class IItem*,
				class IActor*,
				const struct Vector3&,
				const struct Vector3&);
  virtual void OnBulletHitWorld(class IItem*,
				const struct Vector3&,
				const struct Vector3&);
  virtual void OnLog(const char*);
  void Log(const char*);
  virtual void OnMasterServerConnected(bool, const char*, const char*);
  virtual void OnLoginComplete(bool,
			       const char*,
			       bool,
  ...
  virtual void OnJoinGameServerComplete(bool,
					const char*,
					bool,
					const char*,
					uint16_t,
					const char*);
  virtual void OnGameServerConnected(bool,
				     const char*,
				     const struct Vector3&,
				     const struct Rotation&);
  ...
  class Actor* CreateRemoteActorByNameWithOwner(const std::string&,
						bool,
						class IActor*);
